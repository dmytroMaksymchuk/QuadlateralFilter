...
from QuadlateralFilter.helpers.gaussianHelper import add_gauss_noise_2d
from QuadlateralFilter.quadlateral.quadlateralFilter2D import quadrateral_filter_2d, add_gauss_noise_shape
from QuadlateralFilter.trilateral.trilateralFilter2D import match_shape, trilateral_filter_2d
...
def get_data():
    ...
    # noised_Z = add_gauss_noise_shape(Z, 3)
    noised_Z = Z

    noised_Z = noised_Z.clip(0, 255).astype(np.uint8)

    bilateral = cv.bilateralFilter(noised_Z, 19, sigmaIntensity, sigmaSpatial)
    result, quad = quadrateral_filter_2d(noised_Z, sigmaSpatial, sigmaIntensity)
    result = result.clip(0, 255).astype(np.uint8)

    trilat = trilateral_filter_2d(noised_Z, sigmaSpatial, sigmaIntensity)

    # trilat = Z
    # bilateral = Z

    return Z, noised_Z, bilateral, trilat, result, quad
...
def get_fig(z_data):
    ...
    fig.update_layout(scene=dict(zaxis=dict(range=[-10, 210])))
...
original, noised, bilateral, trilat, filtered, quad = get_data()
app = Dash()
app.layout = html.Div([
    html.Div([
        ...
        dcc.RadioItems(options=['original', 'noise', 'bilateral', 'trilateral', 'filtered', 'diff'], value='original', id='choose-graph'),
